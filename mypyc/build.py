...
def resolve_cfile_deps(
    ...
    # Worklist of (search_dir, is_angled, header_name). search_dir is the includer's
    # directory; for the
    # initial cfile it is the cfile's dir, for a transitively-included header it is that
    ...
    while worklist:
        ...
        for base in search_bases:
            ...
            # Recurse only into headers. Some lib-rt sources are pulled in as `#include
            # "init.c"` etc.;
            # those do not resolve under target_dir so they get filtered out before we
...
def mypyc_build(
    ...
    *,
    ...
    # Header resolution is deferred to a second pass: a header in one group may include
    # a header
    # generated by another group, so resolving here misses cross-group deps for groups
    ...
    # Second pass: assemble each group's .c filenames and resolve transitive deps now
    # that every group's
