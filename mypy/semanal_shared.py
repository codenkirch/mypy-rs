...
def find_dataclass_transform_spec(node: Node | None) -> DataclassTransformSpec | None:
    ...
    # The spec only lives on the function/class definition itself, so we need to unwrap
    # down to that
    # point
    ...
    if isinstance(node, TypeInfo):
        # Search all parent classes to see if any are decorated with
        # `typing.dataclass_transform`
        ...
        # > If dataclass_transform is applied to a class, dataclass-like semantics will
        # be assumed
        # > for any class that directly or indirectly derives from the decorated class
