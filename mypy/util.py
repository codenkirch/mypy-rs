...
# Issue #564: wire remaining pure util.rs functions to production.
try:
    from type_kernel import (
        rust_bytes_to_human_readable_repr as _rust_bytes_to_human_readable_repr,
        rust_correct_relative_import as _rust_correct_relative_import,
        rust_count_stats as _rust_count_stats,
        rust_find_python_encoding as _rust_find_python_encoding,
        rust_get_mypy_comments as _rust_get_mypy_comments,
        rust_get_prefix as _rust_get_prefix,
        rust_get_unique_redefinition_name as _rust_get_unique_redefinition_name,
        rust_hash_digest as _rust_hash_digest,
        ...
        rust_soft_wrap as _rust_soft_wrap,
        rust_split_module_names as _rust_split_module_names,
        rust_split_target as _rust_split_target,
        rust_split_words as _rust_split_words,
        rust_time_spent_us as _rust_time_spent_us,
        rust_trim_source_line as _rust_trim_source_line,
        rust_unmangle as _rust_unmangle,
        rust_unnamed_function as _rust_unnamed_function,
    )

    _HAS_RUST_UTILS = True
except ImportError:
    _rust_bytes_to_human_readable_repr = None  # type: ignore[assignment]
    _rust_correct_relative_import = None  # type: ignore[assignment]
    _rust_count_stats = None  # type: ignore[assignment]
    _rust_find_python_encoding = None  # type: ignore[assignment]
    _rust_get_mypy_comments = None  # type: ignore[assignment]
    _rust_get_prefix = None  # type: ignore[assignment]
    _rust_get_unique_redefinition_name = None  # type: ignore[assignment]
    _rust_hash_digest = None  # type: ignore[assignment]
    _rust_hash_digest_bytes = None  # type: ignore[assignment]
    _rust_hash_path_stem = None  # type: ignore[assignment]
    _rust_is_dunder = None  # type: ignore[assignment]
    _rust_is_sunder = None  # type: ignore[assignment]
    _rust_is_stub_package_file = None  # type: ignore[assignment]
    _rust_is_sub_path_normabs = None  # type: ignore[assignment]
    _rust_module_prefix = None  # type: ignore[assignment]
    _rust_plural_s = None  # type: ignore[assignment]
    _rust_short_type = None  # type: ignore[assignment]
    _rust_soft_wrap = None  # type: ignore[assignment]
    _rust_split_module_names = None  # type: ignore[assignment]
    _rust_split_target = None  # type: ignore[assignment]
    _rust_split_words = None  # type: ignore[assignment]
    _rust_time_spent_us = None  # type: ignore[assignment]
    _rust_trim_source_line = None  # type: ignore[assignment]
    _rust_unmangle = None  # type: ignore[assignment]
    _rust_unnamed_function = None  # type: ignore[assignment]
    _HAS_RUST_UTILS = False
...
# VT100 color code processing was added in Windows 10 Threshold 2.
# Everyone should have a version newer than this. Windows 8 and below
# are not supported but are either going out of support or few % of market.
...
def is_dunder(name: str, exclude_special: bool = False) -> bool:
    ...
    """
    if _HAS_RUST_UTILS:
        return _rust_is_dunder(name, exclude_special)
...
def split_module_names(mod_name: str) -> list[str]:
    ...
    """
    if _HAS_RUST_UTILS:
        return _rust_split_module_names(mod_name)
    out = [mod_name]
    while "." in mod_name:
...
def module_prefix(modules: Iterable[str], target: str) -> str | None:
    if _HAS_RUST_UTILS:
        result = _rust_module_prefix(list(modules), target)
...
def split_target(modules: Iterable[str], target: str) -> tuple[str, str] | None:
    if _HAS_RUST_UTILS:
        result = _rust_split_target(list(modules), target)
        if result is None:
            return None
...
def find_python_encoding(text: bytes) -> tuple[str, int]:
    ...
    if _HAS_RUST_UTILS:
        result = _rust_find_python_encoding(text)
...
def bytes_to_human_readable_repr(b: bytes) -> str:
    ...
    """
    if _HAS_RUST_UTILS:
        return _rust_bytes_to_human_readable_repr(b)
    return repr(b)[2:-1]
...
def decode_python_encoding(source: bytes) -> str:
    ...
    if _HAS_RUST_UTILS:
        try:
            from type_kernel import rust_decode_python_encoding as _rust_decode

            return _rust_decode(source)
        except Exception:
            pass
...
def trim_source_line(line: str, max_len: int, col: int, min_width: int) -> tuple[str, int]:
    ...
    """
    if _HAS_RUST_UTILS:
        result = _rust_trim_source_line(line, max_len, col, min_width)
        if result is not None:
            return (result[0], int(result[1]))
...
def get_mypy_comments(source: str) -> list[tuple[int, str]]:
    if _HAS_RUST_UTILS:
        result = _rust_get_mypy_comments(source)
        if result is not None:
...
def get_prefix(fullname: str) -> str:
    ...
    if _HAS_RUST_UTILS:
        return _rust_get_prefix(fullname)
    ...
    for attr in get_class_descriptors(old.__class__):
        ...
        try:
            if hasattr(old, attr):
                ...
                delattr(new, attr)
        # No way to distinguish getsetdescriptors that allow writes from
        # ones that don't, so ignore AttributeErrors if needed.
        # TODO: getsetdescriptors that act like properties?
...
def is_sub_path_normabs(path: str, dir: str) -> bool:
    ...
    """
    if _HAS_RUST_UTILS:
        return _rust_is_sub_path_normabs(path, dir)
...
if sys.platform == "linux" or sys.platform == "darwin":
    ...
    def os_path_join(path: str, b: str) -> str:
        # Based off os.path.join, simplified to str-only, 2 args,
        # mypyc-compilable.
...
def get_unique_redefinition_name(name: str, existing: Container[str]) -> str:
    ...
    """
    if _HAS_RUST_UTILS:
        result = _rust_get_unique_redefinition_name(name, list(existing))  # type: ignore[call-overload]
        if result is not None:
            return result
...
def soft_wrap(msg: str, max_len: int, first_offset: int, num_indent: int = 0) -> str:
    ...
    """
    if _HAS_RUST_UTILS:
        return _rust_soft_wrap(msg, max_len, first_offset, num_indent)
...
def hash_digest_bytes(data: bytes) -> bytes:
    ...
    """
    if _HAS_RUST_UTILS:
        return _rust_hash_digest_bytes(data)
...
def is_typeshed_file(typeshed_dir: str | None, file: str) -> bool:
    if _HAS_RUST_UTILS:
        from type_kernel import rust_is_typeshed_file as _rust_is_typeshed_file

        return _rust_is_typeshed_file(typeshed_dir, file=file)
...
def is_stdlib_file(typeshed_dir: str | None, file: str) -> bool:
    if _HAS_RUST_UTILS:
        from type_kernel import rust_is_stdlib_file as _rust_is_stdlib_file

        return _rust_is_stdlib_file(typeshed_dir, file=file)
...
def is_stub_package_file(file: str) -> bool:
    if _HAS_RUST_UTILS:
        return _rust_is_stub_package_file(file)
...
def unnamed_function(name: str | None) -> bool:
    if _HAS_RUST_UTILS:
        return _rust_unnamed_function(name)
