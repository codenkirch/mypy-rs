...
write_raw_bytes: Callable[[WriteBuffer, bytes], None] | None
try:
...
# Literals can contain bytes and enum-values: we special-case both and
# store the value as a string, relying on the fallback type to say how
...
# Fingerprint of the tvar nodes one serialization walk touched: each entry
# is (node, TypeVarId, meta_level) captured at store time.
_TvarFingerprint = tuple[tuple["TypeVarLikeType", "TypeVarId", int], ...]
...
def _clear_type_wire_cache() -> None:
    ...
    # The accumulator pins the last session's tvar nodes and their bounds;
    # drop them with the entries. Never clear mid-session: the in-flight
    # fingerprint depends on it.
    if _type_wire_cache_session_depth == 0:
        _type_wire_cache_tvars.clear()
...
def _wire_cache_storable(t: Type) -> bool:
    """Whether `t` may be stored as a wire-cache root entry.

    False for bare tvar roots: a stored bare tvar would hit in
    `_write_type_cached` before the recording branch fires, and the
    splice would skip the walk that records it, starving the enclosing
    fingerprint. Re-walking a bare tvar costs a few dozen bytes.
    """
    return not isinstance(t, TypeVarLikeType)


def _record_tvar_fingerprint(t: TypeVarLikeType) -> None:
    """Record one tvar node for the enclosing session's fingerprint."""
    key = id(t)
    if key not in _type_wire_cache_tvars:
        _type_wire_cache_tvars[key] = (t, t.id, t.id.meta_level)


def _tvar_fingerprint() -> _TvarFingerprint | None:
    """Session fingerprint; None means the walk touched no tvar node."""
    if not _type_wire_cache_tvars:
        return None
    return tuple(_type_wire_cache_tvars.values())


def _tvar_fingerprint_valid(fp: _TvarFingerprint) -> bool:
    """True while every recorded (node, TypeVarId, meta_level) holds."""
    for node, tvid, level in fp:
        if node.id is not tvid or tvid.meta_level != level:
            return False
    return True
...
_serialize_stats = {
    ...
    "writes": 0,
    "tvar": 0,
    # Serves and rejects of tvar-fingerprinted entries (funnel + splice).
    "tvar_hits": 0,
    "tvar_reject": 0,
    "bytes": 0,
    "mirror": 0,
...
class Type(mypy.nodes.Context):
    ...
    def can_be_true_default(self) -> bool:
        # Native seam retired (#1640): base default is True.
        return True
...
class AnyType(ProperType):
    ...
    def __init__(
        ...
        # If this Any was created from interacting with another Any,
        # record the source and use it in reports.
...
class Instance(ProperType):
    ...
    def __init__(
        ...
        *,
        ...
        #
        # Currently most of mypy will ignore this field and will continue to treat this
        # type like
    ...
    def copy_modified(
        ...
        *,
        ...
        # Native seam retired (#1739): the Python body is an O(1) rebuild,
        # the wire round trip was 17x that (whole-tree codec).
        new = Instance(
            typ=self.type,
...
class CallableType(FunctionLike):
    ...
    def copy_modified(
        ...
        # Native seam retired (#1739): the Python body already assembles
        # every field, so the `changes` bookkeeping only fed the seam.
    ...
    def max_possible_positional_args(self) -> int:
        ...
        # Native seam retired (#1640): O(n) scalar read.
    ...
    def is_generic(self) -> bool:
        # Native seam retired (#1640): list truthiness check.
...
class TupleType(ProperType):
    ...
    def length(self) -> int:
        # Native seam retired (#1640): O(1) len read.
...
class TypedDictType(ProperType):
    ...
    def copy_modified(
        ...
        *,
        ...
        # Native seam retired (#1739): the Python body is an O(1) rebuild
        # and it is the only path that can express `item_names`.
        if fallback is None:
...
class UnionType(ProperType):
    ...
    def can_be_true_default(self) -> bool:
        # Native seam retired (#1640): any() over cached flags.
...
def _set_native_truthiness_resolver(resolver: Any) -> None:
    """Retired (#1640): kept as no-op for test compatibility."""
...
def _serialize_type_for_visitor(t: Type) -> bytes:
    ...
    # Bare tvars are never cache roots (they would shadow the recording
    # branch in _write_type_cached for every enclosing serialization).
    if _wire_cache_enabled() and not isinstance(t, TypeVarLikeType) and (not isinstance(t, Instance) or t.type_ref is None):  # type: ignore[misc]
        if _serialize_stats_on:
            _serialize_stats["writes"] += 1
...
def _encode_no_arg_instance(t: Type, buf_cls: type[WriteBuffer]) -> bytes | None:
    ...
    is guaranteed to return a None fingerprint. Skipping it avoids the
    session management overhead (depth tracking, accumulator reset,
    function call) for the most common serialize target.
...
def has_recursive_types(typ: Type) -> bool:
    ...
    # Native seam retired (#1640): serialize+walk cost exceeds walk.
...
def flatten_nested_unions(
    ...
    # Native seam retired (#1739): Python scans the list for flattens to
    # do, the wire path serialized every tree to reach the same O(n) scan.
...
def _write_type_cached(t: Type, data: WriteBuffer) -> None:
    ...
    """
    global _type_wire_cache_session_depth
    if not _type_wire_cache_enabled or write_raw_bytes is None:
        if write_raw_bytes is None and _type_wire_cache_session_depth > 0:
            # No-librt fallback: funnels still store at session exit, so
            # the fingerprint must stay complete for those stores to be
            # safe.
            if isinstance(t, (TypeVarType, ParamSpecType, TypeVarTupleType)):
                _record_tvar_fingerprint(t)
        t.write(data)
        return
    key = id(t)
    entry = _type_wire_cache.get(key)
    ...
    if _type_wire_cache_session_depth > 0:
        ...
        t.write(data)
        return
    # Outermost: isolate bytes via temp buffer, record tvars, store with
    ...
    try:
        ...
        _type_wire_cache_session_depth -= 1
    blob = tmp.getvalue()
    fp = _tvar_fingerprint()
    if not isinstance(t, Instance) or t.type_ref is None:  # type: ignore[misc]
        _type_wire_cache[key] = (t, blob, fp)
    write_raw_bytes(data, blob)


def _serialize_with_taint_check(
    t: Type, buf: WriteBuffer
) -> tuple[bytes, _TvarFingerprint | None]:
    """Serialize `t` to `buf` and return (bytes, tvar_fingerprint).

    Wraps `t.write(buf)` in an outermost wire-cache session so nested
    `_write_type_cached` calls write directly to `buf` (no individual
    caching) while every tvar node the walk touches is recorded. The
    fingerprint (None for tvar-free types) gates cache reuse: a stored
    entry is served only while every recorded (node, TypeVarId,
    meta_level) triple still holds.
    """
    global _type_wire_cache_session_depth
    _type_wire_cache_session_depth += 1
    _type_wire_cache_tvars.clear()
    try:
        ...
        _type_wire_cache_session_depth -= 1
    if isinstance(t, (TypeVarType, ParamSpecType, TypeVarTupleType)):
        # A tvar root is written by t.write directly here (the bare-tvar
        # branch in _write_type_cached never fires), so record it now.
        _record_tvar_fingerprint(t)
    return buf.getvalue(), _tvar_fingerprint()
...
def write_type_map(data: WriteBuffer, value: dict[str, Type]) -> None:
    ...
    for key in value:
        ...
        # Route through _write_type_cached so a tvar inside TypedDict
        # items / ExtraAttrs attrs is fingerprinted like every other
        # child; with the cache off this is exactly value[key].write(data).
        _write_type_cached(value[key], data)


# This cyclic import is unfortunate, but to avoid it we would need to move away all uses
# of get_proper_type() from types.py. Majority of them have been removed, but few
# remaining
