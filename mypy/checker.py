...
class TypeChecker(NodeVisitor[None], TypeCheckerSharedApi, SplittingVisitor):
    ...
    def check_match_args(self, var: Var, typ: Type, context: Context) -> None:
        ...
        if not self.scope.active_class():
            return
        # Native type_kernel seam: the type predicate is a pure bool over
        # one wire Type; the note emission stays here. None falls through
        # to the pure-Python body below.
        if (
            _CHECKER_HAS_TYPE_KERNEL
            and _native_checker_active
            and _rust_check_match_args is not None
        ):
            try:
                result = _rust_check_match_args(_serialize_type_for_checker(typ))
    ...
    def intersect_instances(
        ...
        try:
            ...
            if local_errors.has_new_errors():
                ...
                reversed_pair = (instances[1], instances[0])
        ...
        for n2 in m2:
            if literal_hash(n1) == literal_hash(n2):
                if coalesce_any and isinstance(get_proper_type(m1[n1]), AnyType):
                    ...
                    combined[n1] = make_simplified_union([m1[n1], m2[n2]])
    return combined
...
def is_typeddict_type_context(lvalue_type: Type) -> bool:
    if (
        ...
        try:
            ...
            result = _rust_is_typeddict_type_context(_native_checker_resolver, type_bytes)
            if result is not None:
                return result
        except (AssertionError, NotImplementedError):
            pass
