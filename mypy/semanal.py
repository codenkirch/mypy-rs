...
class SemanticAnalyzer(
    ...
    def _native_prepare_method_signature(
        ...
        """
        self_type: ProperType | None = None
            ...
            try:
                ...
                if metaclass_name is not None:
                    ...
                    if isinstance(sym_node, Var) and sym_node.type is not None:
                        var_type = get_proper_type(sym_node.type)
    ...
    def visit_import_from(self, imp: ImportFrom) -> None:
        if _SEMANAL_VISITOR_HAS_KERNEL and _native_semanal_visitor_active:
            if _rust_visit_import_from(imp, self):
                return
        self.statement = imp
    ...
    def visit_import_all(self, i: ImportAll) -> None:
        if _SEMANAL_VISITOR_HAS_KERNEL and _native_semanal_visitor_active:
            if _rust_visit_import_all(i, self):
                return
    ...
    def visit_with_stmt(self, s: WithStmt) -> None:
        if _SEMANAL_VISITOR_HAS_KERNEL and _native_semanal_visitor_active:
            if _rust_visit_with_stmt(s, self):
                return
        self.statement = s
        types: list[Type] = []
    ...
    def _lookup(
        ...
        """
        implicit_name = False
        # Native seam retired (#1723): rust_lookup mirrors this walk but
        # loses 1.8-4.2x per call on every path (1.14M corpus calls).
