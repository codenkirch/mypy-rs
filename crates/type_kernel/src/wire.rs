...
/// Collect every `type_ref` string from a wire-format `Type` blob.
/// Returns `None` on decode failure. The caller checks each ref
/// against the `typeinfo_map` keys to decide if the fixup walk can
/// skip the Python `TypeTranslator` (all refs resolve) or must defer.
fn collect_type_refs(t: &Type, refs: &mut Vec<String>) {
    match t {
        Type::Instance {
            type_ref,
            args,
            ...
            if let Some(lkv) = last_known_value {
                collect_type_refs(lkv, refs);
            }
            if let Some(ea) = extra_attrs {
                for v in ea.attrs.values() {
        ...
        Type::CallableType {
            ...
            collect_type_refs(fallback, refs);
            if let Some(it) = instance_type {
                collect_type_refs(it, refs);
            }
            for at in arg_types {
                collect_type_refs(at, refs);
            }
            collect_type_refs(ret_type, refs);
            for v in variables {
                collect_type_refs(v, refs);
            }
            if let Some(tg) = type_guard {
                collect_type_refs(tg, refs);
            }
            if let Some(ti) = type_is {
                collect_type_refs(ti, refs);
            }
        }
        Type::Overloaded { items } => {
            for item in items {
        ...
        Type::TupleType {
            ...
            collect_type_refs(partial_fallback, refs);
        }
        Type::TypedDictType {
            fallback, items, ..
        } => {
        ...
        Type::TypeVarType {
            ...
            collect_type_refs(upper_bound, refs);
            collect_type_refs(default, refs);
        }
        Type::ParamSpecType {
            prefix,
            upper_bound,
        ...
        Type::TypeVarTupleType {
            ...
            collect_type_refs(tuple_fallback, refs);
            collect_type_refs(upper_bound, refs);
            collect_type_refs(default, refs);
        }
        Type::LiteralType { fallback, value: _ } => {
            collect_type_refs(fallback, refs);
        }
        Type::UnpackType { typ } => {
            collect_type_refs(typ, refs);
        }
        Type::Parameters(p) => {
            for at in &p.arg_types {
                collect_type_refs(at, refs);
            }
        ...
        Type::AnyType { source_any, .. } => {
            if let Some(sa) = source_any {
                collect_type_refs(sa, refs);
            }
        }
        Type::NoneType
        | Type::ErasedType
...
/// `#[pyfunction]` entry: given wire-format `Type` bytes and a set of
/// valid typeinfo_map fullnames, returns `true` if every `type_ref`
/// in the decoded type is in the set. Returns `false` on decode
/// failure or any missing ref. The Python caller uses this to decide
/// whether to skip the expensive `_TypeRefFixer` walk.
#[pyfunction]
pub(crate) fn rust_verify_type_refs(bytes: &[u8], valid_refs: Vec<String>) -> PyResult<bool> {
    let mut buf = ReadBuffer::new(bytes);
    ...
    collect_type_refs(&typ, &mut refs);
    let valid: std::collections::HashSet<&str> = valid_refs.iter().map(|s| s.as_str()).collect();
    Ok(refs.iter().all(|r| valid.contains(r.as_str())))
}
...
mod tests {
    ...
    // ----- collect_type_refs / rust_verify_type_refs -----

    fn collect_refs(t: &Type) -> Vec<String> {
        let mut refs = Vec::new();
        collect_type_refs(t, &mut refs);
        refs
    }

    #[test]
    fn collect_refs_instance_simple() {
        let t = Type::Instance {
            type_ref: "a.A".to_string(),
            args: Vec::new(),
            last_known_value: None,
        ...
        assert_eq!(collect_refs(&back), vec!["a.A".to_string()]);
    }

    #[test]
    fn collect_refs_instance_generic() {
        let t = Type::Instance {
            type_ref: "a.A".to_string(),
            args: vec![
                Type::Instance {
        ...
        assert_eq!(
            collect_refs(&back),
            vec!["a.A", "builtins.int", "builtins.str"]
        );
    }

    #[test]
    fn collect_refs_callable_type() {
        let t = Type::CallableType {
            fallback: Box::new(Type::Instance {
                type_ref: "builtins.function".to_string(),
                args: Vec::new(),
        ...
        assert_eq!(
            collect_refs(&back),
            vec!["builtins.function", "builtins.int", "builtins.str"]
        );
    }

    #[test]
    fn collect_refs_union_type() {
    ...
    fn collect_refs_tuple_type() {
        ...
        let back = round_trip(&t);
        assert_eq!(collect_refs(&back), vec!["builtins.int", "builtins.tuple"]);
    }

    #[test]
    fn collect_refs_none_type_has_no_refs() {
        let back = round_trip(&Type::NoneType);
        assert!(collect_refs(&back).is_empty());
    }

    #[test]
    fn collect_refs_type_type_unwraps_item() {
        let t = Type::TypeType {
            item: Box::new(Type::Instance {
                type_ref: "a.A".to_string(),
                args: Vec::new(),
        ...
        let back = round_trip(&t);
        assert_eq!(collect_refs(&back), vec!["a.A"]);
    }

    // --- rust_verify_type_refs ---

    fn write_bytes(t: &Type) -> Vec<u8> {
        let mut buf = WriteBuffer::new();
        write_type(&mut buf, t).expect("write_type failed");
        buf.into_bytes()
    }
    ...
    fn verify_refs_all_present() {
        ...
        let bytes = write_bytes(&t);
        let valid = vec!["a.A".to_string(), "builtins.int".to_string()];
        assert!(rust_verify_type_refs(&bytes, valid).unwrap());
    }

    #[test]
    fn verify_refs_missing_ref() {
        let t = Type::Instance {
            type_ref: "a.A".to_string(),
            args: vec![Type::Instance {
                type_ref: "builtins.int".to_string(),
        ...
        let bytes = write_bytes(&t);
        // Only "a.A" is valid; "builtins.int" is missing.
        let valid = vec!["a.A".to_string()];
        assert!(!rust_verify_type_refs(&bytes, valid).unwrap());
    }

    #[test]
    fn verify_refs_empty_valid_set() {
        let t = Type::Instance {
            type_ref: "a.A".to_string(),
            args: Vec::new(),
            last_known_value: None,
        ...
        let bytes = write_bytes(&t);
        assert!(!rust_verify_type_refs(&bytes, Vec::new()).unwrap());
    }

    #[test]
    fn verify_refs_no_refs_always_true() {
        let bytes = write_bytes(&Type::NoneType);
        // NoneType carries no type_ref; any valid set (even empty) passes.
        assert!(rust_verify_type_refs(&bytes, Vec::new()).unwrap());
    }

    #[test]
    fn verify_refs_bad_bytes_returns_false() {
        let bad = [0xFF, 0x00, 0x42];
        assert!(!rust_verify_type_refs(&bad, Vec::new()).unwrap());
    }

    #[test]
    fn verify_refs_empty_bytes_returns_false() {
        let bad: [u8; 0] = [];
        assert!(!rust_verify_type_refs(&bad, Vec::new()).unwrap());
    }
}
